...
iteration/v{major}.{minor}/01-product/
        v{major}.{minor}-requirement.md
        v{major}.{minor}-feature-specification.md  ← 可选（v1.1+ 通常合并到 requirement）
        v{major}.{minor}-prototype.html             ← 可交互 UI 原型（人类使用）
        v{major}.{minor}-prototype-skeleton.html    ← LLM 输入用的最小骨架
...
iteration/v{major}.{minor}/04-implementation/
        v{major}.{minor}-source-code.md             ← 实施主记录（含 ISSUE 表）
...
| 模板 | 文件 | 用途 | 派生产物 |
|---|---|---|---|
| **product** | `product.md` | 需求 + 功能规格（合并）| `v{N}-requirement.md` + `v{N}-feature-specification.md` |
...
| `FS-NNN` | 功能规格（v1.0 之前单独，v1.1+ 合并入 requirement）| `FS-001` |
...
#   - iteration/v1.0/01-product/v1.0-requirement.md
#   - iteration/v1.0/01-product/v1.0-feature-specification.md
```
...
| 阶段 | 文件 | 限制 |
|---|---|---|
| 01-product | `feature-specification.md` | v1.1+ 推荐 inline 进 requirement.md（FS-NNN 作为 FR-NNN 子项）|
| 04-implementation | `issue-fixes.md` | 已合并入 `source-code.md §5 已知与清单`（v1.1+ 不再单独）|
| 06-rc-review-release | `project-overview.md` + `user-guide.md` + `README.md` | 已合并为 `release-notes.md` |
| `prototype-skeleton.html` | 1381 → 635 行（删除 mock data + render 业务逻辑）| 人类用 `prototype.html` 完整版 |
