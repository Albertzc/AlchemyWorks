...
- After `05-review-release` passes, the workflow automatically regenerates the instance root `README.md` from the approved current-version requirement under `## 当前系统功能说明` and updates `<!-- workflow:workspace-readme-version: v{major}.{minor} -->`. `workspace/README.md` only documents workspace responsibilities and operating conventions.
- When the next consecutive version is successfully created, archive that completed predecessor to `iteration/archive/v{major}.{minor}/`. The predecessor must first pass the `05-review-release` gate and the instance root README refresh check; the first version (`v1.0`) has no predecessor to archive.
