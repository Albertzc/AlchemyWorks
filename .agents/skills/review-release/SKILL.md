...
Use `workflow-governance` to run `index` after artifact changes. Before submitting the release artifact, merge the completed version's user-visible functionality into `workspace/README.md` under `## 当前系统功能说明` and update `<!-- workflow:workspace-readme-version: v{major}.{minor} -->`. Submit the artifact and workspace documentation for human review; do not set `status: Approved`. After human approval, use `workflow-governance` to run `validate --stage 05-review-release` under the `stage-gate` policy. The next version creation then archives the completed predecessor according to `manage-iteration`; it refuses to archive if the workspace README refresh is missing.
