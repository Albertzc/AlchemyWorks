...
Use `workflow-governance` to run `index` after artifact changes. Before submitting the release artifact, review the instance root `README.md` under `## 当前系统功能说明`; a successful `validate --stage 05-review-release` regenerates it from the Approved requirement and updates `<!-- workflow:workspace-readme-version: v{major}.{minor} -->`. Keep `workspace/README.md` focused on workspace responsibilities and operating conventions. Submit the artifact and instance documentation for human review; do not set `status: Approved`. After human approval, use `workflow-governance` to run `validate --stage 05-review-release` under the `stage-gate` policy. The next version creation then archives the completed predecessor according to `manage-iteration`; it refuses to archive if the root README refresh is missing.
