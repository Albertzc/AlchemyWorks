...
这些目录的结构和说明文件属于工作流本身；其中后续放入的需求、版本产物和业务代码属于实例项目，不属于本清单。`init-instance` 创建目录，`sync` 只同步下表登记的说明文件，不覆盖实例内容。
...
| `workspace/` | 目录本身 | 实例项目真实业务代码的承载目录；其中 `README.md` 由 05-review-release 通过后的当前版本 requirement 自动生成，记录当前系统功能说明和版本标记。 |
