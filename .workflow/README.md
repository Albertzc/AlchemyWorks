...
When the CLI is run from the framework source repository, pass `--project-root` before the subcommand. The selected instance project is then the only root used for product documents and generated `.workflow` state; the framework source tree is used only for framework code, templates, Skills, and protection checks. When the workflow CLI has been synchronized into an instance project, omitting `--project-root` keeps the existing local-root behavior.

`workspace/README.md` is instance-owned: the workflow does not generate or synchronize it. The root `README.md` documents the shared workflow framework and is checked only for current Skills and scripts; `workspace/README.md` documents the instance project's current functionality and carries the per-iteration refresh marker. Archive policy: `v1.0` has no predecessor. For a later version, `init-version` first validates the active predecessor through `05-review-release`, checks the workspace README refresh marker, creates the successor skeleton, and only then moves that predecessor to `iteration/archive/`. A failed predecessor gate or workspace README check removes the empty successor skeleton and leaves the predecessor and archive unchanged.
...
- `task-runs/` — task completion conclusions and gate/context metadata. This is local instance-project audit evidence and is ignored by default; commit selected records manually when needed.
- `task-runs/history/` — immutable task completion history. This is local instance-project audit evidence and is ignored by default; commit selected records manually when needed.
