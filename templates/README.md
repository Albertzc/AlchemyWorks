...
> 所有权：`templates/` 属于工作流本身，不属于任何单个实例。模板由工作流仓库维护，并在同步时复制到实例供 Agent 使用；实例中的同步副本默认忽略，不应直接作为业务提交修改。
...
HTML 原型必须保留模板顶部的审核 frontmatter。生成时使用 `status: draft`；人工审核完成后填写 `reviewer`、`reviewed_at`、`review_notes`，并由人工将审核通过的原型设为 `status: Approved`。工作流会在门禁中强制检查这些字段。
